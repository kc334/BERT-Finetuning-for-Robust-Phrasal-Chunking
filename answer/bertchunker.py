...
class FinetuneTagger:
    ...
    ### Added
    # Add noise 
    # noise_prob determines the chance to add noise
    def add_noise(self, sentence, noise_prob=0.4):
        # only add "add, drop_character" noise type since other noise type decrease score
        # modify noise_list to see effect of other noise 
        ...
        for word in sentence:
            ...
            if torch.rand(1) < noise_prob:
                ...
                #Randomly swap a character in a word
                #uncomment to see effect 
                """
                elif noise_type == "character_swap":
                    chars = list(word)
                    index_1 = torch.randint(0, len(word), (1,)).item()
                    index_2 = torch.randint(0, len(word), (1,)).item()
                    # Swap the characters at the randomly generated indices
                    chars[index_1], chars[index_2] = chars[index_2], chars[index_1]
    ...
    ### Added
    # Augments the data with noise for adversarial training
    # Ratio determines how much data to add, leads to higher runtime as ratio increases
    ...
    def train(self):
        ...
        ### Added
        # call augment_data to augment the training data 
        self.augment_data()
